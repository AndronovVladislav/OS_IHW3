#include "../Other/utils.h"  /* TCP echo server includes */
...
int main(int argc, char *argv[]) {
    ...
    int obsrvSock = AcceptTCPConnection(servSock);
    for (;;) {
        ...
        if ((processID = fork()) < 0) {
            ...
            HandleTCPClient2(clntSock, clntAmount, obsrvSock);
        ...
        int stop;
        if (recv(obsrvSock, &stop, sizeof(stop), 0) < 0) {
